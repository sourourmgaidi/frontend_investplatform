...
export class ServiceRequestService {
  private apiUrl = 'http://localhost:8089/api/service-requests';
  ...
  /**
   * Demander la modification d'un service
   */
  ...
  /**
   * Demander la suppression d'un service
   */
   ...
   * Voir mes demandes
   */
  getMyRequests(): Observable<{ success: boolean; requests: ServiceRequest[]; count: number }> {
    return this.http.get<{ success: boolean; requests: ServiceRequest[]; count: number }>(
   ...
   */
  getRequestById(requestId: number): Observable<{ success: boolean; request: ServiceRequest }> {
    return this.http.get<{ success: boolean; request: ServiceRequest }>(
   ...
   */
  getPendingRequests(): Observable<{ success: boolean; requests: ServiceRequest[]; count: number }> {
    return this.http.get<{ success: boolean; requests: ServiceRequest[]; count: number }>(
   ...
   */
  getAllRequests(type?: string, status?: string): Observable<{ success: boolean; requests: ServiceRequest[]; count: number }> {
  ...
  /**
   * Voir les demandes par type
   */
  getRequestsByType(type: 'EDIT' | 'DELETE'): Observable<{ success: boolean; requests: ServiceRequest[]; count: number }> {
    return this.http.get<{ success: boolean; requests: ServiceRequest[]; count: number }>(
      `${this.apiUrl}/admin/type/${type}`,
      { headers: this.getHeaders() }
    );
  }
   ...
   */
  getAdminRequestById(requestId: number): Observable<{ success: boolean; request: ServiceRequest }> {
    return this.http.get<{ success: boolean; request: ServiceRequest }>(
